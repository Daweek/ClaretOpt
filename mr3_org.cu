...
extern "C" void MR3calcnacl(double x[], int n, int atype[], int nat,
		double pol[], double sigm[], double ipotro[], double pc[], double pd[],
		double zz[], int tblno, double xmax, int periodicflag, double force[]) {
	int i, j;
	...
	if (n != n_bak) {
		...
		if (nalloc != nalloc_bak) {
			...
			CUDA_SAFE_CALL(
					cudaMalloc((void**) &d_x,
							sizeof(VG_XVEC) * (nalloc + NTHREOPT2)));
			CUDA_SAFE_CALL(
					cudaMalloc((void**) &d_force,
							sizeof(float) * (nalloc + NTHREOPT2)*3));

#if 1
			free(vec);
			if ((vec = (VG_XVEC *) malloc(sizeof(VG_XVEC) * (nalloc + NTHREOPT2)))
					== NULL) {
...
#endif
			free(forcef);
			if ((forcef = (float *) malloc(sizeof(float) * nalloc * 3))
					== NULL) {
		...
		CUDA_SAFE_CALL(
				cudaMemcpyToSymbol(c_matrix, matrix,
						sizeof(VG_MATRIX) * nat * nat));
	...
	CUDA_SAFE_CALL(
			cudaMemcpy(d_x, vec,
					sizeof(VG_XVEC) * ((n + NTHREOPT2 - 1) / NTHREOPT2 * NTHREOPT2),
...
//  nacl_kernel_gpu_kadai12<<< grid, threads >>>(d_x,n,nat,NULL,xmaxf,d_force);
	...
	CUDA_SAFE_CALL(
			cudaMemcpy(forcef, d_force, sizeof(float) * n * 3,
